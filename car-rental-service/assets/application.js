...
function displayNotice(a, b) {
	clearNotice();
	var c = $("<div>").text(a).addClass("flash " + b);
	$("header").after(c)
}
...
function clearNotice() {
	...
	function (a, b) {
		...
		b.DEFAULTS = {
			...
			e.hoverState = "in", this.timeout = setTimeout(function () {
			...
			var g = d[0].offsetWidth,
				h = d[0].offsetHeight;
			b == "top" && h != f && (c = !0, a.top = a.top + f - h);
			if (b == "bottom" || b == "top") {
				var i = 0;
				a.left < 0 && (i = a.left * -2, a.left = 0, d.offset(a), g = d[0].offsetWidth, h = d[0].offsetHeight), this.replaceArrow(i - e + g, g, "left")
			...
			function e() {
				var b = setTimeout(function () {
					c.filter(":not(.in)").off(a.support.transition.end).detach()
				}, 500);
				c.one(a.support.transition.end, function () {
					clearTimeout(b), c.filter(":not(.in)").detach()
				})
			}
			var b = this,
				c = this.tip(),
			...
			return c.removeClass("in"), a.support.transition && this.$tip.hasClass("fade") ? e() : c.detach(), this.$element.trigger("hidden.bs." + this.type), this
	...
	function () {
		...
		a = jQuery, a.payment = {}, a.payment.fn = {}, a.fn.payment = function () {
			...
			var b;
			return a.prop("selectionStart") != null && a.prop("selectionStart") !== a.prop("selectionEnd") ? !0 : (typeof document != "undefined" && document !== null ? (b = document.selection) != null ? typeof b.createRange == "function" ? b.createRange().text : void 0 : void 0 : void 0) ? !0 : !1
		}, n = function (b) {
			var c = this;
			...
			if (!/^\d+$/.test(f)) return;
			d = a(c.currentTarget), j = d.val(), e = b(j + f), g = (j.replace(/\D/g, "") + f).length, i = 16, e && (i = e.length[e.length.length - 1]);
			if (g >= i) return;
			if (d.prop("selectionStart") != null && d.prop("selectionStart") !== j.length) return;
			e && e.type === "amex" ? h = /^(\d{4}|\d{4}\s\d{6})$/ : h = /(?:^|\s)(\d{4})$/;
			if (h.test(j)) return c.preventDefault(), d.val(j + " " + f);
			if (h.test(j + f)) return c.preventDefault(), d.val(j + f + " ")
		}, f = function (b) {
			var c, d;
			c = a(b.currentTarget), d = c.val();
			if (b.meta) return;
			if (c.prop("selectionStart") != null && c.prop("selectionStart") !== d.length) return;
			if (b.which === 8 && /\s\d?$/.test(d)) return b.preventDefault(), c.val(d.replace(/\s\d?$/, ""))
		}, i = function (b) {
			var c, d, e;
			d = String.fromCharCode(b.which);
			if (!/^\d+$/.test(d)) return;
			c = a(b.currentTarget), e = c.val() + d;
			if (/^\d$/.test(e) && e !== "0" && e !== "1") return b.preventDefault(), c.val("0" + e + " / ");
			if (/^\d\d$/.test(e)) return b.preventDefault(), c.val("" + e + " / ")
		}, j = function (b) {
			var c, d, e;
			d = String.fromCharCode(b.which);
			if (!/^\d+$/.test(d)) return;
			c = a(b.currentTarget), e = c.val();
			if (/^\d\d$/.test(e)) return c.val("" + e + " / ")
		}, k = function (b) {
			var c, d, e;
			d = String.fromCharCode(b.which);
			if (d !== "/") return;
			c = a(b.currentTarget), e = c.val();
			if (/^\d$/.test(e) && e !== "0") return c.val("0" + e + " / ")
		}, g = function (b) {
			var c, d;
			if (b.meta) return;
			c = a(b.currentTarget), d = c.val();
			if (b.which !== 8) return;
			if (c.prop("selectionStart") != null && c.prop("selectionStart") !== d.length) return;
			if (/\s\/\s?\d?$/.test(d)) return b.preventDefault(), c.val(d.replace(/\s\/\s?\d?$/, ""))
		}, r = function (a) {
			var b;
			return a.metaKey || a.ctrlKey ? !0 : a.which === 32 ? !1 : a.which === 0 ? !0 : a.which < 33 ? !0 : (b = String.fromCharCode(a.which), !!/[\d\s]/.test(b))
		}, p = function (c) {
			var d, e, f, g;
			d = a(c.currentTarget), f = String.fromCharCode(c.which);
			if (!/^\d+$/.test(f)) return;
			if (l(d)) return;
			return g = (d.val() + f).replace(/\D/g, ""), e = b(g), e ? g.length <= e.length[e.length.length - 1] : g.length <= 16
		}, q = function (b) {
			var c, d, e;
			c = a(b.currentTarget), d = String.fromCharCode(b.which);
			if (!/^\d+$/.test(d)) return;
			if (l(c)) return;
			e = c.val() + d, e = e.replace(/\D/g, "");
			if (e.length > 6) return !1
		}, o = function (b) {
			var c, d, e;
			c = a(b.currentTarget), d = String.fromCharCode(b.which);
			if (!/^\d+$/.test(d)) return;
			return e = c.val() + d, e.length <= 4
		}, s = function (b) {
			var c, e, f, g, h;
			c = a(b.currentTarget), h = c.val(), g = a.payment.cardType(h) || "unknown";
			if (!c.hasClass(g)) return e = function () {
				var a, b, c;
				c = [];
				for (a = 0, b = d.length; a < b; a++) f = d[a], c.push(f.type);
				return c
			}(), c.removeClass("unknown"), c.removeClass(e.join(" ")), c.addClass(g), c.toggleClass("identified", g !== "unknown"), c.trigger("payment.cardType", g)
			...
			var b, c, d, e;
			return a = a.replace(/\s/g, ""), e = a.split("/", 2), b = e[0], d = e[1], (d != null ? d.length : void 0) === 2 && /^\d+$/.test(d) && (c = (new Date).getFullYear(), c = c.toString().slice(0, 2), d = c + d), b = parseInt(b, 10), d = parseInt(d, 10), {
				month: b,
				year: d
			}
			...
			var d, e, f, g;
			return typeof b == "object" && "month" in b && (g = b, b = g.month, c = g.year), !b || !c ? !1 : (b = a.trim(b), c = a.trim(c), /^\d+$/.test(b) ? /^\d+$/.test(c) ? parseInt(b, 10) <= 12 ? (c.length === 2 && (f = (new Date).getFullYear(), f = f.toString().slice(0, 2), c = f + c), e = new Date(c, b), d = new Date, e.setMonth(e.getMonth() - 1), e.setMonth(e.getMonth() + 1, 1), e > d) : !1 : !1 : !1)
		}, a.payment.validateCardCVC = function (b, d) {
			var e, f;
			return b = a.trim(b), /^\d+$/.test(b) ? d ? (e = b.length, u.call((f = c(d)) != null ? f.cvcLength : void 0, e) >= 0) : b.length >= 3 && b.length <= 4 : !1
		}, a.payment.cardType = function (a) {
			var c;
			return a ? ((c = b(a)) != null ? c.type : void 0) || null : null
		}, a.payment.formatCardNumber = function (a) {
			var c, d, e, f;
			return c = b(a), c ? (e = c.length[c.length.length - 1], a = a.replace(/\D/g, ""), a = a.slice(0, +e + 1 || 9e9), c.format.global ? (f = a.match(c.format)) != null ? f.join(" ") : void 0 : (d = c.format.exec(a), d != null && d.shift(), d != null ? d.join(" ") : void 0)) : a
		}
	...
	function (a) {
		a.fn.spin = function (b, c) {
			...
			if (Spinner) return this.each(function () {
				var e = a(this),
					f = e.data();
				f.spinner && (f.spinner.stop(), delete f.spinner), b !== !1 && (typeof b == "string" && (b in d ? b = d[b] : b = {}, c && (b.color = c)), f.spinner = (new Spinner(a.extend({
					color: e.css("color")
				}, b))).spin(this))
			});
	...
	function (a) {
		function f(b, d) {
			if (b.indexOf("#") === 0) a(b).clone(!0).appendTo(c.empty()).show();
			else {
				if (d) {
					var e = a("<iframe />").attr({
						src: b,
		...
		function g(e, f) {
			a("#opacity-fade").show();
			var g = c.outerWidth(!0),
				h = {},
				i = {};
			if (c.is(":visible") || d) return;
			d = !0;
			switch (e) {
			case "left":
				c.css({
			...
			b.animate(h, f), c.show().animate(i, f, function () {
				d = !1
			}), b.addClass("body-no-scroll")
		}
		var b, c;
		a(function () {
			b = a("body"), c = a("#pageslide"), c.click(function (a) {
				a.stopPropagation()
			}), a(document).bind("click keyup", function (b) {
				if (b.type == "keyup" && b.keyCode != 27) return;
				c.is(":visible") && !c.data("modal") && a.pageslide.close()
			})
		});
		var d = !1,
			e;
		a.fn.pageslide = function (b) {
			var d = this;
			d.click(function (d) {
				var f = a(this),
					g = a.extend({
				...
				d.preventDefault(), d.stopPropagation(), c.is(":visible") && f[0] == e ? a.pageslide.close() : (a.pageslide(g), e = f[0])
			})
		}, a.fn.pageslide.defaults = {
			speed: 200,
			direction: "right",
			modal: !1,
			iframe: !0,
			href: null
		}, a.pageslide = function (b) {
			var d = a.extend({}, a.fn.pageslide.defaults, b);
			c.is(":visible") && c.data("direction") != d.direction ? a.pageslide.close(function () {
				f(d.href, d.iframe), g(d.direction, d.speed)
			}) : (f(d.href, d.iframe), c.is(":hidden") && g(d.direction, d.speed)), c.data(d)
		}, a.pageslide.close = function (c) {
			var e = a("#pageslide"),
				f = e.outerWidth(!0),
				g = e.data("speed"),
				h = {},
				i = {};
			if (e.is(":hidden") || d) return;
			d = !0;
			...
			e.animate(i, g, function () {
				e.hide(), b.removeClass("body-no-scroll")
			}), a("#opacity-fade").hide(), d = !1
		}
	}(jQuery),
	function (a) {
		...
		h.defaults = {}, a.removeCookie = function (b, c) {
			return a.cookie(b) !== undefined ? (a.cookie(b, "", a.extend({}, c, {
				expires: -1
			})), !0) : !1
		}
	}), $(document).ready(function () {
		$("[data-disable-on-click='true']").click(function () {
			$this = $(this), $this.attr("disabled", "disabled"), $this.parents("form").submit()
		})
	}), jQuery(function (a) {
		var b = b || [],
		...
		a('[data-toggle~="tooltip"]').tooltip({
			container: "body"
		}), a("#help_pageslide").bind("click", function () {
			b.push(["_setAccount", "UA-26573244-1"]), b.push(["_trackEvent", "Help", c])
		}).pageslide({
			direction: "left",
			speed: "500"
		}), a(document).on("click", ".selectable", function () {
			var b = a(this),
				c = b.attr("data-input"),
				d = b.attr("data-value");
			a('[data-input="' + c + '"]').removeClass("selected"), b.addClass("selected");
			var e = b.parent("form"),
				f = e.children("input" + c);
			a(f).val(d)
		}), a(document).on("click", ".radio_select", function () {
			var b = a(this).data("input");
			a(".radio_select").filter('[data-input="' + b + '"]').removeClass("selected").filter(this).addClass("selected")
		}), a("[data-focus]").click(function () {
			$this = a(this);
			var b = $this.data("focus");
			a(b).focus()
		}), a("form[data-validate]").on("validate", "input[type='text'], textarea, select", function () {
			$this = a(this), $form = $this.parents("form"), $submit = $form.find("input[type='submit']"), $form.find(".invalid").length > 0 ? $submit.attr("disabled", "disabled") : $submit.removeAttr("disabled")
		}), a("form[data-validate]").on("blur", "[data-validation='required']", function () {
			$this = a(this);
			var b = $this.val().length > 0;
			b ? ($this.tooltip("destroy"), $this.removeClass("invalid").trigger("validate")) : ($this.tooltip({
				container: "body"
			}), $this.tooltip("show"), $this.addClass("invalid").trigger("validate"))
		})
	}), jQuery(function (a) {
		function c(a) {
		...
		var b = ["com", "net", "org", "edu"];
		a("#domain_droplet_id").on("change", function () {
			a(this).val() != "" && a(this).val() != undefined && a("#domain_ip_address").val(a(this).val())
		}), a(".edit_domain_record").on("keydown", 'input[type="text"]', function (b) {
			b.which == 13 && a(this).closest("form").submit()
		}), a("#add_cname_record #domain_record_data").on("blur keyup", function () {
			var b = a(this),
				d = b.val();
			c(d) ? b.tooltip({
				title: 'In order to redirect to another domain this must end in a period. "' + d + '."'
			}).tooltip("show") : b.tooltip("destroy")
		})
	}), jQuery(function () {
		function c(a, b) {
			return $("<div>").addClass("flash alert " + b).text(a)
		}

		function d(a) {
			var b = a.data("available-settings");
			$settings.map(function () {
				var a = $(this),
					c = a.data("setting");
				$.inArray(c, b) != -1 ? a.show() : (a.children("label").removeClass("checked"), a.hide())
			})
		}

		function e(a) {
			var b = a.data("available-sizes");
			$sizes.map(function () {
				$size = $(this);
				var a = $size.data("id");
				$.inArray(a, b) != -1 ? ($size.removeClass("disabled"), $size.tooltip("destroy")) : ($size.removeClass("selected").addClass("disabled"), $size.tooltip({
					title: "Due to high demand and capacity restrictions we have temporarily disabled this size in this region.",
					container: "body"
				}), $size.find("input").removeAttr("checked"))
			})
		}

		function f(a) {
			var b = a.data("available-regions");
			$regions.map(function () {
				$region = $(this);
				var a = $region.data("id");
				$.inArray(a, b) != -1 ? h($region) : g($region)
			})
		}

		function g(a) {
			a.removeClass("selected").addClass("disabled");
			var b = a.data("available-size-names") || [],
				c = b.join(", ") || "None",
				d = "Due to high demand we have temporarily disabled this region for this size. " + c + " are available.";
			a.tooltip({
				title: d,
				container: "body"
		...
		function h(a) {
			a.removeClass("disabled"), a.tooltip("destroy")
		}

		function i(a, b) {
			$("#my_images input, #my_destroyed_droplets input, #my_backups input").each(function (c, d) {
				var e = $(d),
					f = a.data("id"),
					g = a.data("image-upgrade"),
					h = b.data("size"),
					i = e.data("disk"),
					k = e.parent().data("upgraded-regions");
				if ($.inArray(f, e.data("region-id")) == -1) {
					var l = "Droplets can only be created using images in the same region, you can transfer this image on the Images page.";
					j(e, l)
				} else if (i > h) {
					var l = "Cannot create a droplet with a smaller disk (" + h + "GB) than the image (" + i + "GB).";
					j(e, l)
				} else if (g && k.indexOf(f) < 0) {
					var l = "Droplets in the selected region can only be created using upgraded images, please upgrade on the images page.";
					j(e, l)
				} else e.parent().removeClass("disabled").tooltip("destroy")
			})
		}

		function j(a, b) {
			a.parent().addClass("disabled").removeClass("selected").tooltip("destroy").tooltip({
				title: b,
				container: "body"
			}).find("input").removeAttr("checked")
		}
		$distros = $("#new_droplet .distro"), $sizes = $("#sizes .size").not(".disabled"), $region_inputs = $('#new_droplet [name="droplet_create[region_id]"]'), $regions = $("#new_droplet .region"), $settings = $("#new_droplet .setting"), $(".size.disabled").map(function () {
			var a = $(this),
				b = a.data("restrictions")[0];
			a.tooltip({
				title: b,
				container: "body"
			})
		}), $hostnameInput = $("#new_droplet [name='droplet_create[name]']"), $hostnameInput.focus(), $hostnameInput.tooltip("show"), setTimeout(function () {
			$hostnameInput.tooltip("destroy")
		}, 5e3);
		var a = $(".region.selected"),
			b = $(".size.selected");
		f(b), d(a), i(a, b), $("#new_droplet #sizes").on("click", ".size:not(.disabled)", function () {
			var a = $(this);
			f(a), i($(".region.selected"), a)
		}), $("#new_droplet #regions").on("click", ".region:not(.disabled)", function () {
			var a = $(this);
			$region_inputs.attr("checked", !1), a.find("input").attr("checked", "checked"), e(a), d(a), $regions.removeClass("selected").filter(this).addClass("selected"), i(a, $(".size.selected"))
		}), $("#new_droplet #distributions").on("click", ".distro", function () {
			$(".distro").removeClass("active").filter(this).addClass("active")
		}), $("#new_droplet #ssh_keys").on("click", "label", function () {
			$(this).toggleClass("active"), $("#ssh_keys").find(".active").length > 0 ? ($("#root_password_emailed").addClass("hidden"), $("#root_password_not_emailed").removeClass("hidden")) : ($("#root_password_emailed").removeClass("hidden"), $("#root_password_not_emailed").addClass("hidden"))
		}), $("#new_droplet #settings label").on("click", function () {
			$(this).toggleClass("checked")
		}), $("#new_droplet .distro.deprecation-warning").on("click", function () {
			var a = c($(this).data("deprecation-notice"), "deprecation_warning");
			$("#distributions").append(a)
		}), $("#new_droplet .distro:not(.deprecation-warning)").on("click", function () {
			$(".flash.alert.deprecation_warning").remove()
		}), $("#new_droplet [name='droplet_create[name]']").on("blur keyup", function () {
			var a = !1,
				b = "",
				d = $(this);
			this.value.length < 1 ? b = "Invalid Hostname: Your hostname must not be blank." : /^[a-zA-Z0-9]?[a-z0-9A-Z\.\-]*[a-z0-9A-Z\.]$/.exec(this.value) ? this.value.length > 200 ? b = "Invalid Hostname: Your hostname is too long." : (b = "", a = !0) : b = "Invalid Hostname: Your hostname must be alphanumeric characters (no spaces).", d.data("valid", a), $(".flash.hostname_validation").remove(), $submit = $("input[type='submit']"), a ? $submit.removeAttr("disabled") : ($alert = c(b, "hostname_validation"), d.after($alert.clone()), $submit.before($alert.clone()), $submit.attr("disabled", "disabled"))
		})
	}), jQuery(function (a) {
		a(".toggle-listing").on("click", "a", function () {
			return a(".hidden").toggle(), a(".show-listing").toggleClass("shown"), a(".hide-listing").toggleClass("shown"), !1
		}), a("#action-select").on("show", 'a[data-toggle="tab"]', function (b) {
			var c = a(a(b.target).attr("href")),
				d = a(a(b.relatedTarget).attr("href"));
			c.is("ul") ? c.find("li:first a").tab("show") : a("#action-tabs").find(".tabs.active").children(".active").andSelf().removeClass("active"), d.is("ul") && d.children(".active").removeClass("active")
		}), a("#actions-snapshots").on("shown", function () {
			$input = a("#action-take_snapshot input[type='text']"), $input.focus(), $input.tooltip("show"), setTimeout(function () {
				$input.tooltip("destroy")
			}, 5e3)
		}), a('[data-dependent="submit"]').change(function () {
			var b = a(this).data("dependent-selector"),
				c = a(b);
			a(this).val() == "" ? c.attr("disabled", "disabled") : c.removeAttr("disabled")
		}), a("#actions").on("ajax:beforeSend", function (b, c, d) {
			a("#event-processing").addClass("active"), a("#actions").hide()
		}).on("ajax:success", function (b, c, e, f) {
			c.status == "ERROR" ? (d(), displayNotice(c.error, "alert")) : a("#event-processing").trigger("events:poll", c.event_id)
		}).on("ajax:error", function (a, b, c, e) {
			d()
		});
		var b = a("#event-processing"),
			c = b.data("event_id");
		update_event_progress = function (b) {
			a("#pending_event").show(), a("#pending_event_name").text(b.name), a("#pending_event_progress").find(".bar").css({
				width: b.progress + "%"
			})
		};
		var d = function () {
			a("#event-processing").removeClass("active"), a("#actions").show()
		};
		event_poll = function (b, c) {
			c = typeof c != "undefined" ? c : 1e3, setTimeout(function () {
				a.getJSON("/events/" + b).done(function (a) {
					a.status != null ? window.location.reload() : (c += 250, update_event_progress(a), event_poll(b, c))
				})
			}, c)
		}, a("#action-destroy #toggle_scrub_data").on("click", function () {
			a(this).toggleClass("checked")
		}), b.on("events:poll", function (a, b) {
			event_poll(b)
		}), c != undefined && c != "" && b.trigger("events:poll", c)
	}), jQuery(function (a) {
		a("#take_a_snapshot").click(function () {
			return a("#snapshot_form").slideToggle(250), !1
		}), a(".image-region .dropdown-toggle").tooltip({
			container: "body"
		})
	}), jQuery(function (a) {
		function b(b, c) {
		...
		function e() {
			c && (displayNotice("Confirmed password does not match.", "alert"), c = !1)
		}
		a("#deactivate_account label, #billing_alerts label").on("click", function () {
			a(this).toggleClass("checked")
		}), a("#change_password_new_password").keyup(function () {
			b(this, "#change_password_confirm_new_password")
		}), a("#change_password_confirm_new_password").keyup(function () {
			b(this, "#change_password_new_password")
		});
		var c = !0
	}), jQuery(function (a) {
		$size_inputs = a('.sizes-selection [name="droplet[size_id]"]'), $sizes_available = a(".sizes-selection .size"), a(".sizes-selection.carousel.slider").carousel({
			interval: !1
		}).on("slide", function () {
			a(this).find(".carousel-control").toggleClass("left").toggleClass("right")
		}), a(".sizes-selection").on("click", ".size", function () {
			if (a(this).hasClass("disabled")) return !1;
			$size_inputs.attr("checked", !1), a(this).find("input").attr("checked", !0), $sizes_available.removeClass("selected").filter(this).addClass("selected")
		})
	}), jQuery(function () {
		function c() {
			var a = this.value;
			...
			$.get("/support/suggest", {
				...
				$("#ticket_topic_suggestions").empty().html(a).show(), b.spin(!1)
			})
		}
		...
		function d(a) {
			var e = this,
				f = [16, 37, 38, 39, 40];
			if ($.inArray(a.which, f) >= 0) return !1;
			if ($(e).val() == "") return b.spin(!1), clearTimeout(d.timeout), $("#ticket_topic_suggestions").empty(), !1;
			d.timeout && clearTimeout(d.timeout), b.data("spinner") || b.spin("promo"), d.timeout = setTimeout(function () {
				c.call(e, a)
			}, 50)
		}
		var a = $("#ticket_topic"),
			b = $("#ticket_topic_suggestion_indicator");
		a.data("suggest") && a.live("keyup", d)
	}), jQuery(function (a) {
		a("#new_user_payment_profile").submit(function () {
			var b = a(this).find("[data-required]"),
				c = b.filter(function (b) {
					return a.trim(a(this).val()) == ""
				}),
				d = a(this).find('input[type="submit"]'),
				e = a("#required-fields-alert").length ? a("#reuired-fields-alert") : a('<p id="required-fields-alert" class="flash alert">Please fill in the fields which are marked in red</p>').hide().insertBefore(this);
			if (c.length > 0) return e.slideDown(), b.removeClass("required-field").filter(c).addClass("required-field"), !1;
			if (a(this).data("submitting") != undefined && a(this).data("submitting") != "") return !1;
			e.fadeOut(), d.attr({
				disabled: "disabled",
				value: "Processing..."
			}).end().data("submitting", !0).submit()
		})
	}), jQuery(function (a) {
		a("#newsletter_subscription_checkbox").change(function () {
			a(this).parent("form").submit()
		}), a("input.cc-num").payment("formatCardNumber").on("blur", function () {
			var b = a(this).val(),
				c = a.payment.validateCardNumber(b);
			b != "" ? c ? a(this).removeClass("invalid").addClass("valid") : a(this).removeClass("valid").addClass("invalid") : a(this).removeClass("valid invalid")
		}), a("input.cc-cvc").payment("formatCardCVC").on("blur", function () {
			var b = a("input.cc-num").val(),
				c = a.payment.validateCardNumber(b),
				d = a.payment.cardType(b),
				e = a(this).val(),
				f = a.payment.validateCardCVC(e, d);
			e != "" ? c && f ? a(this).removeClass("invalid").addClass("valid") : a(this).removeClass("valid").addClass("invalid") : a(this).removeClass("valid invalid")
		}), a("input.cc-exp").payment("formatCardExpiry").on("blur", function () {
			var b = (new Date).getFullYear() + 20,
				c = a(this).val(),
				d = a.payment.cardExpiryVal(c),
				e = a.payment.validateCardExpiry(d.month, d.year);
			if (d.year == NaN || d.year > b) e = !1;
			c != "" ? e ? (a(this).removeClass("invalid").addClass("valid"), a(this).val(d.month + " / " + d.year)) : a(this).removeClass("valid").addClass("invalid") : a(this).removeClass("valid invalid")
		})
	}), jQuery(function (a) {
		a("body.domains.show").length > 0 && (a(".edit_domain_record").find("input[type='text']").each(function () {
			a(this).data("initialValue", a(this).val())
		}), a(".edit_domain_record").on("change", "input[type='text']", function () {
			a(this).val() == a(this).data("initialValue") ? a(this).parent("form").trigger("input:initial_value") : a(this).parent("form").trigger("input:changed"), console.log("changed"), console.log(this)
		}), a(".edit_domain_record").on("input:changed", function () {
			a(this).closest("li").addClass("changed")
		}).on("input:initial_value", function () {
			a(this).closest("li").removeClass("changed")
		}))
	});